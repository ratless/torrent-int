...
BitTorrent остаётся одной из самых устойчивых и масштабируемых P2P-сетей в мире, но при этом её внутреннее устройство DHT, протоколы обмена, поведенческие сигнатуры остаются малоизученными в контексте OSINT-аналитики. Большинство существующих работ либо описывают протокол на абстрактном уровне, либо фокусируются на обнаружении пиратского контента, либо оптимизируют сетевые алгоритмы. Однако систематического, практического подхода к использованию DHT как источника разведывательных данных с чёткими индикаторами риска, методиками обогащения и визуализацией -- практически нет.

Данное исследование представляет собой попытку заполнить этот пробел. В работе детально разбираются: протокол Kademlia и его реализация в BT; методы пассивного и активного сбора данных; обогащение IP-адресов геолокацией, провайдерской информацией и репутационными скорами; графовые модели для обнаружения кластеров согласованной активности; временные паттерны, позволяющие отличать обычную активность от автоматизированной.

**Работа ориентирована на спецов в ИБ, сотрудников правоохранительных органов, OSINT-любителей и всех, кто сталкивается с острой необходимостью исследования вопроса децентрализованных сетей, который на данный момент является мало раскрытым и мало изученным по своей природе.**
...
![Архитектура DHT](./docs/imgs/ru/arch_bittorrent.png)
...
![Структура узла и K-бакеты](./docs/imgs/ru/k-backet.png)
...
![Формат сообщений KRPC](./docs/imgs/ru/krpc.png)
...
![Процесс поиска пира](./docs/imgs/ru/search_peer.png)
...
![Основные BEP-расширения](./docs/imgs/ru/bep_ext.png)
...
![Общий пайплайн сбора данных](./docs/imgs/ru/pipeline.png)
...
![Схема динамики пиров во времени](./docs/imgs/ru/dinamic_peers.png)
...
![Сигнатуры клиентов по peer_id](./docs/imgs/ru/clients.png)
...
![Логическая цепочка выявления аномалий](./docs/imgs/ru/anomalies.png)
...
![Обогащение](./docs/imgs/ru/enrichment.png)
...
![Анализ графов](./docs/imgs/ru/graph_analysis.png)
...
![Динамика](./docs/imgs/ru/daily_dynamics.png)
...
![Индикатор рисков](./docs/imgs/ru/risk_indicator.png)
...
![ROC-кривая](./docs/imgs/ru/roc_curve.png)

---

## Заключение

В ходе работы мы разработали и апробировали полный цикл OSINT-анализа данных, извлекаемых из распределённой хеш-таблицы BT. Основные результаты работы можно свести к следующим пунктам.
Во-первых, мы детально описали архитектуру DHT с выделением в первую очередь неполноты выборки, динамичностью записей, наличием мёртвых узлов и фейковых ответов. Это позволило сформулировать реалистичные ожидания от сбора данных и разработать эмпирически обоснованные параметры краулинга.

Во-вторых, мы показали, что графовый анализ позволяет обнаруживать кластеры скоординированной преступной активности, которые не видны в таблицах. Конкретный пример ботнета из 12 IP (который, к сожалению, я не могу показать из-за политики конфединциальности и этических и правовых норм), начинающих одинаковые торренты с разницей в секунды, демонстрирует, как именно граф выявляет скрытые структуры.

В-четвёртых, анализ временных рядом показал, что нормальные торренты имеют суточную динамику с отклонениями до 15%, в то время как подозрительные демонстрируют резкие скачки до 200% в час, что позволяет отслеживать внешние события в реальном времене.

Работа также имеет ряд ограничений. Во-первых, DHT возвращает не всех пиров, а лишь случайную выборку, поэтому все выводы следует интерпертировать как нижнюю границу наблюдаемой активности. Во-вторых, возможно наличие ложных данных, внесённых злоумышленниками. В-третьих, мы использовали только публичные трекеры и открытые API для обогащения, ибо интеграция в приватными трекерами и закрытыми источниками требует дополнительных согласований.

Направления для будущих исследований включают автоматизацию описанной методологии в виде единого инструмента, интеграцию с другими источниками данных, применение методов машинного обучения для классификации узлов на основе поведенческих профилей и расширение временного горизонта сбора для выявления аномалий.

Я считаю, что представленная работа является первым шагом к использованию DHT-метаданных в OSINT-дисциплине и надеюсь, что она станет основой для дальнейших исследований в этой области и для последующих работ от коллег.
